...
def train(file_path="data/bun_zipper.ply", num_epochs=200):
    ...
    # Camera setup
    camera_pos = torch.tensor([0., 0., 0.5], dtype=torch.float32)  # Close to bunny
    camera_rot = torch.eye(3, dtype=torch.float32)  # Facing forward
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 20 == 0:
            img = render_gaussians(means, colors_pred, cov, opacities, 
                                 camera_pos=camera_pos, camera_rot=camera_rot)
